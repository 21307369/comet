...
async function discoverActiveChanges(projectRoot) {
  ...
  for (const entry2 of entries) {
    ...
    if (!await fileExists3(path17.join(changeDir, ".comet.yaml"))) continue;
    const projection = await readClassicState(changeDir);
    const classic = projection.classic;
    const diagnostic = await inspectClassicChange(changeDir, entry2);
...
function relatedEvidence(utterance, change) {
  ...
  const tokens = change.text.split(/[^a-zA-Z0-9_\-\u4e00-\u9fff/]+/u).map((token) => token.trim().toLowerCase()).filter((token) => token.length >= 4);
...
var classicResumeProbeCommand = async (args) => {
  ...
  const source = input === "--stdin" ? await readStdin2() : input;
