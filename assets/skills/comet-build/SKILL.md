...
**Per-Task Code Search Constraint**:

When executing tasks from tasks.md one by one, before modifying code for each task, the agent **MUST** first search the codebase for existing implementations that could be reused or extended. Skipping this step is prohibited. This constraint applies to both `executing-plans` and `subagent-driven-development` execution modes.

**Search tools** (in priority order, use the first available):
1. `lsp references` / `lsp definition` — search symbol references and implementations when language server is available
2. `ast_grep` — search AST-structured patterns for functions/classes/calls
3. `grep` — full-text search for function names, class names, key logic fragments

**Reuse decision**: Reuse if found, extend in place if partial match (do NOT copy out and modify), create new if none exists and document search process in tasks.md

**Search evidence recording**: Record search results and reuse decision next to the corresponding task entry in `tasks.md`:

```markdown
- [x] Implement user login validation <!-- search: lsp references validateUser → found src/auth/login.ts:42, reuse -->
- [x] Add cache expiration cleanup <!-- search: grep cacheTtl → no match, new -->
```
...
**Subagent mode constraint propagation**: In `subagent-driven-development` mode, this constraint is injected into each background implementer via the dispatch prompt in `comet/reference/subagent-dispatch.md`.
...
**Reuse decision review** (mandatory, independent of review_mode): After all tasks are complete and before running the build → verify phase guard, must verify that reuse decisions recorded in tasks.md are reasonable. If existing code with matching signature was found but the implementer chose NEW, must flag it and request justification or correction. When `review_mode: standard/thorough`, the reviewer performs this check; when `review_mode: off`, the implementer self-reviews and records the conclusion in tasks.md.
...
- Each completed task in tasks.md has code search evidence recorded (`<!-- search: ... -->` format)
- Code committed
