...
2. 对每个未完成任务：
   - **先搜索代码库**（禁止跳过）：使用 `lsp references` / `ast_grep` / `grep` 搜索是否有可复用或可扩展的已有实现。hotfix 场景重点关注 bug 所在模块的已有修复逻辑和相似 bug 的历史修复
   - 根据搜索结果和任务描述修改代码（复用/扩展/新建）
...
**复用决策审查**：运行 build guard 前，必须验证 tasks.md 中记录的复用决策是否合理。如果找到了签名匹配的已有代码但选择了新建，必须说明理由。
