...
按 tasks.md 逐个执行任务时，每个任务修改代码前**必须**先搜索代码库中是否已有可复用或可扩展的实现。跳过此步骤是禁止的。此约束对 `executing-plans` 和 `subagent-driven-development` 两种执行方式均适用。

**搜索工具**（按可用性顺序，使用第一个可用的）：
1. `lsp references` / `lsp definition` — 语言服务器可用时，搜索符号引用和实现
2. `ast_grep` — 搜索 AST 结构匹配的函数/类/调用
3. `grep` — 全文搜索函数名、关键逻辑片段

**复用决策**：找到可复用的就复用，部分匹配就原地扩展（禁止复制出来改），没有就新建并在 tasks.md 说明搜索过程

**搜索证据记录**：每个任务的搜索结果和复用决策记录在 `tasks.md` 对应任务条目旁：

```markdown
- [x] 实现用户登录验证 <!-- search: lsp references validateUser → 找到 src/auth/login.ts:42，复用 -->
- [x] 添加缓存过期清理 <!-- search: grep cacheTtl → 无匹配，新建 -->
```

**代码库知识沉淀**：搜索过程中发现的代码库问题（弃用函数、死代码、已知 bug）记录在 tasks.md 的「已知问题」段落：

```markdown
## 已知问题

- `src/utils/parse.ts:parseYaml()` — 不处理 BOM 头，输入有 BOM 时会崩溃
...
**复用决策审查**（独立于 review_mode，必须执行）：在所有任务完成后、运行 build → verify 阶段守卫前，必须验证 tasks.md 中记录的复用决策是否合理。如果找到了签名匹配的已有代码但 implementer 选择了新建，必须标记并要求说明理由或修正。`review_mode: standard/thorough` 时由 reviewer 执行；`review_mode: off` 时由 implementer 自行审查并在 tasks.md 记录审查结论。
...
- **每完成一个 task**：确认代码搜索已完成（tasks.md 对应任务条目旁有 `<!-- search: ... -->` 证据记录），按当前执行分支和 `review_mode` 完成验收后再勾选对应任务并提交。`subagent-driven-development` 在 `standard` 或 `off` 时不做 per-task reviewer；在 `thorough` 时只按批次或风险边界做合并审查，不做每 task 双审查。所有模式都必须按任务唯一文本完成定向检查。可用 `grep -c '\- \[ \]' tasks.md` 检查剩余未勾选数，无需重新读取整个文件
...
- tasks.md 全部勾选
- tasks.md 中每个已完成任务的条目旁有代码搜索证据记录（`<!-- search: ... -->` 格式）
- 代码已提交
